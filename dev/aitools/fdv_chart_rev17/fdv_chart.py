...
def parse_log_file(file_path, regex_include=None, regex_exclude=None, source_name=None):
    ...
    # ── FDV ICC regex ──────────────────────────────────────────────────────
    # Format: FDV ICC [...path/file.FDV::tname,conds]: DYNAMIC, DUT1, vcc, 5.913298 UA
    _RE_ICC = re.compile(
        r'FDV ICC \[(?P<path>[^\]]+)\.FDV::(?P<tname>[^,\]]+)'
        r'(?:,(?P<conds>[^\]]*))?\]:\s*'
        r'(?P<test_base>\S+),\s*'           # DYNAMIC
        r'(?P<dut>\S+),\s*'                 # DUT1
        r'(?P<param>\S+),\s*'               # vcc
        r'(?P<measurement>[^\s]+)'          # 5.913298
    )
    ...
    def _parse_icc_line(line, lnum):
        m = _RE_ICC.search(line)
        if not m:
            return None
        tname = m.group('tname')
        conds = m.group('conds') or ''
        test_base = m.group('test_base')
        param = m.group('param')
        # Combine test_base + param to create testname (e.g., DYNAMIC_vcc)
        combined_testname = f"{test_base}_{param}".upper()
        ...
        row[_IDX['DUT']]         = m.group('dut').strip()
        row[_IDX['tname']]       = tname
        row[_IDX['testname']]    = combined_testname
        row[_IDX['BLK']]         = _tparam(tname, 'BLK')
        row[_IDX['PAGE']]        = _tparam(tname, r'(?:PAGE|PG)')
        row[_IDX['PAGETYPE']]    = _pagetype(tname)
        row[_IDX['PAGEMAP']]     = _pagemap(tname)
    ...
    try:
        ...
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                elif 'FDV ICC' in line_stripped:
                    row = _parse_icc_line(line_stripped, line_num)
                else:
