...
$chartScript = Join-Path $PSScriptRoot "fdv_chart_rev15\fdv_chart.py"
$chartWorkDir = Join-Path $PSScriptRoot "fdv_chart_rev15"
$chartLog = Join-Path $logDir "fdv_chart_server.log"
$chartErr = Join-Path $logDir "fdv_chart_server.err.log"
"[restart] Launching fdv_chart_rev15.py at $(Get-Date -Format o)" | Out-File -FilePath $chartLog -Append -Encoding utf8

if ($python -and (Test-Path $python)) {
  Write-Output "Starting fdv_chart_rev15.py via venv: $python $chartScript"
  ...
  Write-Output "Starting fdv_chart_rev15.py via py -3.12"
...
Write-Output "fdv_chart_rev15.py server (re)started on port 5058"
