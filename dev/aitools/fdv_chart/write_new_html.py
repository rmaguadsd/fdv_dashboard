"""Write new optimised fdv_chart.html"""
import os
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        #result-table { width: 100%; border-collapse: collapse; font-size: 0.82em; }
        #result-table thead th {
            background: #343a40; color: #fff; padding: 6px 8px;
            text-align: left; position: sticky; top: 0; white-space: nowrap; z-index: 1;
        }
        #result-table tbody td { padding: 4px 8px; border-bottom: 1px solid #e9ecef; white-space: nowrap; }
...
<div id="plot-panel">
    ...
    <div id="plot-area"><canvas id="plot-canvas"></canvas></div>
...
var allRows        = [];          // in-memory row store (primary data source for plots)
...
async function parseFile() {
    ...
    // Async job
    if (!jobResp.job_id) {
        setStatus('Error: ' + (jobResp.error || 'Unknown'), 'error');
        return;
    }
    ...
    while (true) {
        ...
        var sr;
        try {
            var r = await fetch('/parse_status?job=' + jobId);
            sr = await r.json();
        } catch(e) {
            clearInterval(ticker);
            setStatus('Poll error: ' + e, 'error');
            return;
        }
        if (sr.state === 'pending' || sr.state === 'running') continue;
        clearInterval(ticker);
        if (!sr.success) {
            setStatus('Error: ' + (sr.error || 'Parse failed'), 'error');
            return;
        }
...
function _onParseResult(result, filePath, fileUpload, t0) {
    ...
    allRows         = result.rows.slice();   // seed in-memory store
    ...
    setTableInfo('Loading\u2026 (0 / ' + total.toLocaleString() + ' rows) \u00b7 ' +
                 result.headers.length + ' columns \u00b7 ' + fname);

    buildTableHeader(result.headers);
    appendRowsToDOM(result.rows);
    ...
    loadMoreRows(result.csv_id, result.headers, result.rows.length,
                 total, result.headers.length, fname);
}
...
function appendRowsToDOM(rows) {
    ...
    rows.forEach(function(row) {
        ...
        row.forEach(function(cell) {
            cells += '<td>' + escHtml(cell != null ? String(cell) : '') + '</td>';
        });
        tr.innerHTML = cells;
        frag.appendChild(tr);
    });
...
/* ── Progressive loader: feeds allRows[] AND DOM ─────────────────────── */
async function loadMoreRows(csv_id, headers, offset, total, ncols, fname) {
    var CHUNK = 5000;
    while (offset < total) {
        var r   = await fetch('/rows?csv_id=' + csv_id + '&offset=' + offset + '&limit=' + CHUNK);
        var d   = await r.json();
        if (!d.success) break;
        for (var i = 0; i < d.rows.length; i++) allRows.push(d.rows[i]);  // memory store
        appendRowsToDOM(d.rows);
        if (hasActiveFilters()) _doApplyFilters();
...
function parseFilterExpr(raw) {
    ...
    // fallback: substring (or try regex)
    try {
...
function _doApplyFilters() {
    ...
    inputs.forEach(function(inp) {
        ...
        var parsed = parseFilterExpr(val);
        activeCount++;
        if (parsed.type === 'invalid') { inp.classList.add('invalid'); return; }
    ...
    for (var r = 0; r < trs.length; r++) {
        ...
        for (var f = 0; f < filters.length; f++) {
            var cell = cells[filters[f].idx];
            if (!filters[f].fn(cell ? cell.textContent : '')) { pass = false; break; }
        }
        if (pass) { tr.classList.remove('row-hidden'); visCount++; }
    ...
    if (info) {
        ...
        info.textContent = activeCount > 0
            ? base + ' \u2014 ' + visCount.toLocaleString() + ' shown (' +
              activeCount + ' filter' + (activeCount > 1 ? 's' : '') + ')'
            : base;
    }
}

/* ── Plot data source: scan allRows[] in-memory ──────────────────────── */
    ...
    for (var r = 0; r < allRows.length; r++) {
        ...
        if (activeFilters.length > 0) {
            ...
            for (var f = 0; f < activeFilters.length; f++) {
                var cell = row[activeFilters[f].idx];
                if (!activeFilters[f].fn(cell != null ? String(cell) : '')) { pass = false; break; }
            }
            if (!pass) continue;
        }
...
function resetAll() {
    ...
    currentCsvId = null; currentHeaders = []; allRows = [];
    domFullyLoaded = false; filtersVisible = false;
}
...
function drawScatterLine() {
    ...
    // sample large datasets for canvas performance
    var MAX_PTS = 10000;
    var plotPts = points;
    var sampled = false;
    ...
    var filtNote  = hasFilters ? ' \u2014 filtered' : '';
    var sampNote  = sampled ? ' (sampled ' + plotPts.length.toLocaleString() + '/' + points.length.toLocaleString() + ')' : '';
...
function drawCumProba() {
    ...
    var splitDesc = splitCol ? (' split by ' + splitCol + (splitRx ? ' [' + splitRx + ']' : '')) : '';
