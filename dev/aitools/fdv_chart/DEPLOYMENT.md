# FDV Chart - Deployment & Configuration Guide

## Project Structure

```
fdv_chart/
├── fdv_chart.py                   # Main Flask application
├── __init__.py                    # Package initialization
├── requirements.txt               # Python dependencies
├── launch_fdv_chart.ps1          # PowerShell launcher script
...
$env:FDV_CHART_TMPDIR = "D:\fdv_chart_tmp"

# Set secret key for sessions
$env:FDV_CHART_SECRET = "your-secure-secret-key"
...
To change the default port (5058), edit `fdv_chart.py`:
...
```powershell
py -3 fdv_chart.py | Tee-Object -FilePath "fdv_chart.log"
```
...
```python
# Add Chart app alongside Report and Poll
processes.append({
    'name': 'Chart',
    'cmd': python_exe,
    'args': [str(here / 'fdv_chart' / 'fdv_chart.py')],
    'port': 5058
})
```
...
Start-Process powershell -ArgumentList "-NoExit", "-Command `& '.\fdv_chart\launch_fdv_chart.ps1'"
...
1. **Use a Production WSGI Server**:
   ...
   gunicorn -w 4 -b 0.0.0.0:5058 fdv_chart:app
   ```
...
3. **Set Secure Session Key**:
   ```powershell
   $env:FDV_CHART_SECRET = (New-Guid).Guid
   ```
...
Copy-Item "D:\fdv_chart_tmp\*.csv" "D:\Backups\fdv_chart_$backupDate\"
```
