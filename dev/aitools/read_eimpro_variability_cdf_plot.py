...
def _ensure_min_log_ticks_x(ax, min_decades=2.0):
    """Ensure the x-axis on a log scale spans at least `min_decades` decades.
    Expands the upper limit if necessary to create ≥3 major log ticks.
    """
    try:
        x0, x1 = ax.get_xlim()
        eps = 1e-20
        ...
        if span < float(min_decades):
            new_hi = lo + float(min_decades)
            ax.set_xlim(10.0 ** lo, 10.0 ** new_hi)
        try:
...
def _rcdf_xy(values):
    ...
    try:
        ...
        arr.sort()
        n = float(arr.size)
        # Use unique x to avoid vertical steps at duplicates
        x_unique, counts = np.unique(arr, return_counts=True)
        # Tail probability at each unique x is remaining count / n
...
def generate_rcdf_png(df, out_png_path):
    """Generate reverse CDF plot(s) of RBER.
    Facet by readtype (rows) and dut (columns); within each cell, lines are colored by pagetype.
    If readtype/pagetype are missing, fallback to a single overall curve.
    """
    # Try to ensure we have standard column names
    core = ["RBER", "readtype", "pagetype"]
    if not all(c in df.columns for c in core):
        try:
            cols = find_columns(df)
            rename_map = {}
            for k in ("RBER", "readtype", "pagetype"):
                if k in cols:
                    rename_map[cols[k]] = k
            if rename_map:
                df = df.rename(columns=rename_map)
    ...
    for r_idx, rtype in enumerate(readtypes):
        for c_idx, dut in enumerate(duts):
            ...
            ax.tick_params(axis='both', labelsize=8)
            # Ensure at least 3 major ticks on log-x
            try:
                _ensure_min_log_ticks_x(ax, min_decades=2.0)
            except Exception:
                pass
            # Legend inside
            try:
            ...
            try:
                ...
                pass
    # Title with parttype context if present
    ttl = "Reverse CDF of RBER by readtype and pagetype"
    try:
...
def main(argv=None):
    ...
    if fmt in ("png", "both"):
        ...
        # RCDF for overall dataset
        try:
            rcdf_path = os.path.join(out_dir, f"{base}_rcdf.png")
            generate_rcdf_png(df, rcdf_path)
        except Exception as e:
            print("warning: failed to save RCDF PNG: {0}".format(e), file=sys.stderr)
        ...
        try:
            if "parttype" in df.columns:
                ...
                for pt in sorted(df["parttype"].astype(str).unique()):
                    ...
                    # RCDF per parttype
                    try:
                        rcdf_g_path = os.path.join(out_dir, f"{base}_parttype_{safe}_rcdf.png")
                        generate_rcdf_png(sub, rcdf_g_path)
                    except Exception as e:
                        print(f"warning: failed to save per-parttype RCDF for {pt}: {e}", file=sys.stderr)
        except Exception as e:
