...
ALIASES = {
    ...
    "deck": ["deck"],
    # Conditions for per-split outputs
    "VCC": ["vcc", "vdd", "vcore", "core_v", "core"],
    "VCCQ": ["vccq", "vio", "io_v", "io"],
    "TEMP": ["temp", "temperature", "temp_c", "tempc", "t"],
}
...
def load_data(path):
    ...
    # Coerce condition columns to numeric where present (extract numbers like "1.8V", "25C")
    def _extract_float(val):
        ...
        try:
            ...
            try:
                ...
                return float('nan')
    for c in ("VCC", "VCCQ", "TEMP"):
        if c in df.columns:
            try:
                df[c] = df[c].apply(_extract_float)
            except Exception:
                try:
...
def plot_with_seaborn(df, title):
    ...
    g = sns.FacetGrid(
        ...
        sharex=True, sharey=True, margin_titles=False, despine=False, height=3.8, aspect=1.5
    )
    try:
    ...
    try:
        ...
        g.fig.subplots_adjust(top=0.9)
    # No figure-level suptitle; caller may render context outside the image
    return g.fig
...
def plot_with_matplotlib(df, title):
    ...
    for r_idx, rtype in enumerate(readtypes):
        for c_idx, dut in enumerate(duts):
            ...
            ax.set_xlabel("WL", fontsize=9)
            ax.set_ylabel("RBER (log)", fontsize=9)
            # No per-axes title inside the chart
            try:
                ax.set_title("")
            except Exception:
                pass
    ...
    # No figure-level suptitle; use full canvas
...
def _fmt_num_for_suffix(x):
    """Format number into filename-friendly token: 1.2 -> 1p2, 1.25 -> 1p25, nan -> na"""
    try:
        xf = float(x)
        ...
        return "na"
    # round to 3 decimals to avoid noisy duplicates
    xf = round(xf, 3)
    if abs(xf - int(xf)) < 1e-9:
        return str(int(xf))
    s = ("{:.3f}".format(xf)).rstrip('0').rstrip('.')
    return s.replace('.', 'p')


def _build_condition_suffix_and_label(vals):
    """Given a dict like {"VCC":1.2, "VCCQ":1.8, "TEMP":25}, return (suffix, label).
    suffix example: _vcc_1p2_vccq_1p8_temp_25
    label example: VCC=1.2V, VCCQ=1.8V, TEMP=25C
    Only include keys that are present in vals.
    """
    parts = []
    label_parts = []
    order = ["VCC", "VCCQ", "TEMP"]
    for k in order:
        if k in vals:
            v = vals[k]
            tok = _fmt_num_for_suffix(v)
            parts.append(f"{k.lower()}_{tok}")
            try:
                vf = float(v)
                if np.isfinite(vf):
                    unit = "V" if k in ("VCC", "VCCQ") else "C"
                    label_parts.append(f"{k}={vf:g}{unit}")
                else:
                    label_parts.append(f"{k}=NA")
            except Exception:
...
def _iter_condition_subsets(df):
    """Yield tuples (df_sub, suffix, label) for each unique combination of VCC/VCCQ/TEMP present.
    If none of these columns exist, yield a single (df, "", ""). Values are rounded to 3 decimals for grouping.
    """
    cond_present = [c for c in ("VCC", "VCCQ", "TEMP") if c in df.columns]
    if not cond_present:
        yield df, "", ""
        return
    work = df.copy()
    for c in cond_present:
        try:
            ...
            pass
        try:
            work[f"_{c}_norm"] = work[c].astype(float).round(3)
        except Exception:
            work[f"_{c}_norm"] = pd.to_numeric(work[c], errors='coerce').round(3)
    gcols = [f"_{c}_norm" for c in cond_present]
    combos = work[gcols].dropna().drop_duplicates()
    if combos.empty:
        # If all NaN, treat as single subset
        yield df, "", ""
        return
    for _, row in combos.iterrows():
        ...
        for c in cond_present:
            ...
            vals[c] = v
            try:
                colv = pd.to_numeric(work[c], errors='coerce').values
            except Exception:
                colv = work[c].astype(float).values
            mask &= np.isfinite(colv) & (np.isclose(colv, float(v), rtol=0, atol=5e-4))
        sub = df.loc[mask].copy()
        if sub.empty:
            continue
        suffix, label = _build_condition_suffix_and_label(vals)
        yield sub, suffix, label


def _generate_all_outputs_for_subset(df_sub, in_path, out_dir, base, base_suffix, fmt):
    """Generate all PNG/PDF outputs for a given subset, appending base_suffix to file names."""
    title = "RBER vs WL by pagetype - split by readtype and dut"
    # Variability main plot
    if _HAVE_SEABORN:
        fig = plot_with_seaborn(df_sub, title)
    else:
        print("note: seaborn not found; using matplotlib fallback.")
        fig = plot_with_matplotlib(df_sub, title)
    var_path = os.path.join(out_dir, f"{base}{base_suffix}_variability.png")
    try:
        fig.savefig(var_path, dpi=200)
        print(f"saved: {var_path}")
    except Exception as e:
        print(f"warning: failed to save figure: {e}", file=sys.stderr)
    finally:
        try:
            plt.close(fig)
        except Exception:
    ...
    # Summary PNG
    try:
        summary_path = os.path.join(out_dir, f"{base}{base_suffix}_summary.png")
        generate_summary_png(df_sub, summary_path)
    except Exception as e:
        print(f"warning: failed to save summary PNG: {e}", file=sys.stderr)

    # RCDF by pagetype
    try:
        rcdf_pt_path = os.path.join(out_dir, f"{base}{base_suffix}_rcdf.png")
        generate_rcdf_pagetype_only_png(df_sub, rcdf_pt_path)
    except Exception as e:
        print(f"warning: failed to save RCDF (pagetype) PNG: {e}", file=sys.stderr)

    # RCDF by pagetype and deck
    try:
        rcdf_pd_path = os.path.join(out_dir, f"{base}{base_suffix}_rcdf_pagetype_deck.png")
        generate_rcdf_pagetype_deck_png(df_sub, rcdf_pd_path)
    except Exception as e:
        print(f"warning: failed to save RCDF (pagetype/deck) PNG: {e}", file=sys.stderr)

    # RCDF per DUT (pagetype+deck, readtypes side-by-side)
    try:
        generate_rcdf_pagetype_deck_per_dut_pngs(df_sub, out_dir, f"{base}{base_suffix}")
    except Exception as e:
        print(f"warning: failed to save per-DUT RCDF (pagetype/deck) PNGs: {e}", file=sys.stderr)

    # Correlation between readtypes
    try:
        corr_path = os.path.join(out_dir, f"{base}{base_suffix}_correlation.png")
        generate_readtype_correlation_png(df_sub, corr_path)
    except Exception as e:
        print(f"warning: failed to save correlation PNG: {e}", file=sys.stderr)

    # Per-parttype variants if available
    try:
        if "parttype" in df_sub.columns:
            for pt in sorted(df_sub["parttype"].astype(str).unique()):
                sub = df_sub[df_sub["parttype"] == pt]
                if sub.empty:
                    continue
                ...
                var_path = os.path.join(out_dir, f"{base}{base_suffix}_parttype_{safe}_variability.png")
                try:
                ...
                sum_path = os.path.join(out_dir, f"{base}{base_suffix}_parttype_{safe}_summary.png")
                try:
                ...
                try:
                    rcdf_pt_g_path = os.path.join(out_dir, f"{base}{base_suffix}_parttype_{safe}_rcdf.png")
                ...
                try:
                    rcdf_pd_g_path = os.path.join(out_dir, f"{base}{base_suffix}_parttype_{safe}_rcdf_pagetype_deck.png")
                ...
                # RCDF per parttype (pagetype + deck), per DUT
                try:
                    generate_rcdf_pagetype_deck_per_dut_pngs(sub, out_dir, f"{base}{base_suffix}_parttype_{safe}")
                except Exception as e:
                    print(f"warning: failed to save per-DUT per-parttype RCDF (pagetype/deck) for {pt}: {e}", file=sys.stderr)
                # Correlation per parttype
                try:
                    corr_g_path = os.path.join(out_dir, f"{base}{base_suffix}_parttype_{safe}_correlation.png")
    ...
    # Multi-page PDF
    if fmt in ("pdf", "both"):
        pdf_path = os.path.join(out_dir, f"{base}{base_suffix}_per_dut.pdf")
        try:
            generate_pdf(df_sub, pdf_path)
            print(f"saved: {pdf_path}")
        except Exception as e:
            print(f"warning: failed to generate PDF: {e}", file=sys.stderr)
...
def main(argv=None):
    ...
    # Iterate over condition subsets (VCC/VCCQ/TEMP); if none present, yields single subset
    any_emitted = False
    for df_sub, suffix, _label in _iter_condition_subsets(df):
        if df_sub.empty:
            continue
        _generate_all_outputs_for_subset(df_sub, in_path, out_dir, base, suffix, fmt)
        any_emitted = True

    if not any_emitted:
        # Fallback: generate one set without condition suffix
        _generate_all_outputs_for_subset(df, in_path, out_dir, base, "", fmt)
